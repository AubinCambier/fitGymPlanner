...
export const membershipController = {
  ...
  async subscribe(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!pricing.is_active) throw new AppError('This pricing plan is no longer available', 400);
  ...
  async cancel(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!membership) throw new AppError('No active membership to cancel', 404);
